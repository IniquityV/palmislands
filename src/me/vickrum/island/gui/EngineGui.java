...
public class EngineGui extends Engine {
	...
	public void onOpen(InventoryOpenEvent event) {
		...
		Bukkit.getScheduler().runTask(MassiveCore.get(),
				() -> baseGui.getGuiOpenTasks().forEach(guiOpen -> guiOpen.onOpen(event)));
	}

	@EventHandler(priority = EventPriority.LOW)
	public void onClick(InventoryClickEvent event) {
		if (MUtil.isntPlayer(event.getWhoClicked()))
			return;
		...
		if (now - lastClick >= MConf.get().guiClickThrottleDelayMs) {
		...
		if (InventoryUtil.isBottomInventory(event)) {
			...
			if (!hasBottomClick && !isAllowed) {
				MixinMessage.get().msgOne(player, MConf.get().msgGuiLeaveWindowToEdit);
			}
	...
	public void onClose(InventoryCloseEvent event) {
		...
		Bukkit.getScheduler().runTask(MassiveCore.get(),
				() -> baseGui.getGuiCloseTasks().forEach(guiClose -> guiClose.onClose(event)));
	}
