...
import me.vickrum.island.extra.TxtUtil;
import me.vickrum.island.gui.IGuiTypeBuilder;
...
public enum GuiType implements IGuiTypeBuilder {
	...
	INVENTORY(9, -1, InventoryType.CHEST), HOPPER(5, 1, InventoryType.HOPPER), DISPENSER(3, 3, InventoryType.DISPENSER);
	...
	@Override
	public Inventory build(Player player, String guiName, List<String> guiFormat, String... replacements)
			throws MassiveException {
		if (guiName == null || guiName.isEmpty()) {
			throw new MassiveException().addMsg(MConf.get().msgGuiNameNotSet);
		}

		if (guiFormat.isEmpty()) {
			throw new MassiveException().addMsg(MConf.get().msgGuiFormatNotSet);
		}
		...
		if (rowLength != getRowLength()) {
			throw new MassiveException().addMsg(MConf.get().msgGuiRowLengthNotSame);
		}

		if (getRows() > 0 && rows != getRows()) {
			throw new MassiveException().addMsg(MConf.get().msgGuiTooManyRows);
		}
	...
	public static GuiType getFromFormat(List<String> guiFormat) throws MassiveException {
		...
		return Arrays.stream(values())
				.filter(guiType -> guiType.getRows() == rows && guiType.getRowLength() == rowLength).findFirst()
				.orElse(INVENTORY);
	}
}